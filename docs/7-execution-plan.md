...
- [x] `.env`가 `.gitignore`에 등록되어 있음
- [x] `.env.example`에 `POSTGRES_CONNECTION_STRING=postgresql://postgres:{비밀번호}@localhost:5432/todolist` 키만 포함되어 있음
...
- [x] `\d todos` 실행 시 8개 컬럼 (`todo_id`, `user_id`, `category_id`, `title`, `description`, `due_date`, `is_completed`, `created_at`) 확인
- [x] `\d+ todos`에서 `todos.user_id` FK ON DELETE = `CASCADE` 확인
- [x] `\d+ todos`에서 `todos.category_id` FK ON DELETE = `SET DEFAULT` 확인
- [x] `\d+ categories`에서 `categories.user_id` FK ON DELETE = `CASCADE` 확인
- [x] `todos.category_id`의 DEFAULT 값이 `1`로 설정되어 있음
...
- [x] `schema.sql` 재실행 후에도 "일반"의 `category_id`가 항상 `1`로 유지됨
- [x] "일반" 카테고리 재삽입 시 `uq_categories_name_default` 제약으로 오류 발생하여 중복 차단 확인
...
- [x] `user_id IS NULL`인 기본 카테고리 이름 중복 삽입 시 `uq_categories_name_default` 오류 발생 확인
- [x] 기본 카테고리와 동일한 이름을 일반 사용자로 삽입 시 허용됨 확인 (부분 인덱스 분리)
...
- [x] `idx_todos_user_completed`가 `(user_id, is_completed)` 복합 인덱스임 확인
- [x] `idx_todos_created_at`이 `(created_at DESC)` 내림차순 인덱스임 확인
- [x] `EXPLAIN SELECT * FROM todos WHERE user_id = 1 ORDER BY created_at DESC;` 에서 Index Scan 사용 확인
...
- [x] 테스트 데이터 정리 후 DB가 시드 데이터만 남은 상태로 복원 확인
...
- [x] 잘못된 `DB_PASSWORD`로 기동 시 연결 실패 에러 즉시 출력 확인
- [x] Pool에 `max`, `idleTimeoutMillis`, `connectionTimeoutMillis` 값 명시 확인
- [x] Repository 파일에서만 pool을 import하고 Controller·Service에서 직접 import하지 않음 확인
...
- [x] `package.json`의 `scripts`에 `"db:init"` 명령 등록 또는 README에 실행 명령 명시 확인
- [x] README DB 설정 섹션만 따라도 신규 개발자가 로컬 DB를 정상 기동 가능한 수준 확인
...
- [x] `__tests__/unit/`, `__tests__/integration/`, `__tests__/fixtures/` 디렉토리 생성
- [x] `.env.example`에 `PORT`, `NODE_ENV`, `POSTGRES_CONNECTION_STRING`, `JWT_*`, `BCRYPT_SALT_ROUNDS` 항목 모두 포함
- [x] `.env`가 `.gitignore`에 등록되어 있음
- [x] `npm run dev` 실행 시 "Server running on port 3000" 출력 확인
- [x] `npm test` 실행 시 jest가 실행됨 확인
...
- [x] 잘못된 DB 비밀번호로 기동 시 연결 실패 에러 출력 및 프로세스 종료 확인
- [x] Repository 파일에서 `const pool = require('../config/db')`로 Pool import 확인
...
- [x] 존재하지 않는 라우트 호출 시 404 응답 확인
- [x] `NODE_ENV=production`일 때 응답에 `stack` 필드 미포함 확인
...
- [x] `hashPassword(plain)` — `BCRYPT_SALT_ROUNDS`(10) 적용하여 해시 생성 확인
- [x] `comparePassword(plain, hash)` — 일치 시 `true`, 불일치 시 `false` 반환 확인
- [x] 단위 테스트에서 정상 케이스 및 예외 케이스(만료 토큰, 잘못된 시크릿, 비밀번호 불일치) 모두 통과
...
- [x] `validate(schema)` 미들웨어 — `schema.body/params/query` 검사 실패 시 `AppError(400, 'VALIDATION_ERROR', ...)` + `details` 배열 전달 확인
- [x] 인증 헤더 없이 보호 라우트 호출 시 `{ success: false, error: { code: "UNAUTHORIZED" } }` 401 응답 확인
...
- [x] **UC-01 / PRD 3.2** — 이메일 형식 오류, 비밀번호 8자 미만 등 유효성 실패 시 400 + 필드별 오류 메시지 반환 확인
- [x] **UC-02 / BR-01** — `POST /api/auth/login` 올바른 자격증명 시 200 + `{ success: true, data: { accessToken, refreshToken, user } }` 반환 확인
- [x] **UC-02** — Access Token payload에 `userId`, `email` 포함 및 만료 1시간 확인
- [x] **UC-02 / SC-02 [E-01]** — 잘못된 자격증명 시 401 + `{ code: "UNAUTHORIZED" }` 반환 확인
- [x] **UC-03** — `POST /api/auth/logout` 호출 시 200 + `{ success: true, data: null }` 반환 확인
- [x] **토큰 갱신** — `POST /api/auth/refresh`에 유효한 Refresh Token(`Authorization: Bearer`) 전달 시 새 Access Token 반환 확인
- [x] **SC-02 [E-03]** — 만료된 Refresh Token으로 `/refresh` 호출 시 401 반환 확인
- [x] `__tests__/integration/auth.routes.test.js` — 회원가입 → 로그인 → 보호 API 접근 흐름 통합 테스트 통과
...
- [x] **SC-03 [E-01]** — 현재 비밀번호 불일치 시 400 + 오류 메시지 반환 확인
- [x] **UC-14 / BR-07** — `DELETE /api/users/me`에 비밀번호 전달 시 204 반환 및 Todo·Category·User 전체 삭제 확인
- [x] **SC-04 [E-01]** — 탈퇴 시 비밀번호 불일치 시 400 반환 확인
- [x] **PRD 4.3** — 탈퇴 후 동일 이메일 로그인 시 401 반환하여 계정 완전 삭제 확인
- [x] **트랜잭션** — 탈퇴 처리 중 오류 발생 시 전체 롤백 확인 (원자성)
- [x] 인증 없이 `PATCH/DELETE /api/users/me` 호출 시 401 반환 확인
- [x] `__tests__/unit/user.service.test.js` — 비밀번호 검증 및 탈퇴 트랜잭션 단위 테스트 통과
...
- [x] **PRD 3.2** — 카테고리명 빈 문자열 또는 30자 초과 시 400 + 필드 오류 메시지 반환 확인
- [x] **UC-06 / BR-04** — 사용자 정의 카테고리 수정 시 200 + 수정된 정보 반환 확인
- [x] **UC-06 / SC-06 [E-01]** — 기본 카테고리 수정 요청 시 403 + `{ code: "FORBIDDEN" }` 반환 확인
- [x] **UC-07 / BR-04** — 기본 카테고리 삭제 요청 시 403 반환 확인
- [x] **UC-07 / SC-07** — 할일이 있는 카테고리 삭제 시 해당 할일의 `category_id`가 "일반"으로 변경되고 204 반환 확인 (DB 직접 조회 검증)
- [x] 타인 소유 카테고리 수정·삭제 요청 시 403 또는 404 반환 확인
- [x] `__tests__/integration/category.routes.test.js` — 카테고리 삭제 후 할일 이동 흐름 통합 테스트 통과
...
- [x] **UC-09 / BR-02** — `PATCH /api/todos/:todoId` 일부 필드만 수정 시 200 + 수정된 할일 반환 확인
- [x] **UC-09 / SC-08 [E-01]** — 타인 소유 할일 수정 요청 시 403 또는 404 반환 확인
- [x] **UC-10 / BR-02** — `DELETE /api/todos/:todoId` 본인 소유 삭제 시 204 + DB 레코드 삭제 확인
- [x] **UC-10 / SC-09 [E-01]** — 타인 소유 할일 삭제 요청 시 403 또는 404 반환 확인
- [x] **UC-11 / BR-02** — `PATCH /api/todos/:todoId/completion` 호출 시 `isCompleted` 토글 및 200 반환 확인
- [x] **UC-12 / BR-02,06** — `GET /api/todos` 호출 시 본인 할일 전체 등록일시 내림차순 200 반환 확인
- [x] **UC-12 / SC-10** — `?categoryId=`, `?isCompleted=`, `?dueDateFrom=&dueDateTo=` 각각 및 조합 필터링 정확히 동작 확인
- [x] **UC-13 / BR-02** — `GET /api/todos/:todoId` 본인 소유 상세 조회 시 전체 정보 200 반환 확인
- [x] **SC-11 [E-02]** — 삭제된 할일 ID 조회 시 404 + `{ code: "NOT_FOUND" }` 반환 확인
- [x] `__tests__/unit/todo.service.test.js` — BR-02 소유권 검증 로직 단위 테스트 통과
- [x] `__tests__/integration/todo.routes.test.js` — 타인 토큰으로 할일 접근 시 403 통합 테스트 통과
